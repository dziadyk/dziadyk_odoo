...
class DoctorScheduleWizard(models.TransientModel):
    ...
    def constrains_period(self):
        for obj in self:
            ...
            """
            day_to_shift = 1
            if obj.schedule_shift == 'week':
                day_to_shift = 7
            shift1 = True
            cur_date = obj.begin_date
            while cur_date <= obj.end_date:

                cur_start = obj.start_time_1
                cur_finish = obj.finish_time_1
                if not shift1:
                    cur_start = obj.start_time_2
                    cur_finish = obj.finish_time_2

                rec_count = self.env['hr.hosp.doctor.schedule'].search_count(
                    [('date', '=', cur_date),
                     ('doctor_id', '=', obj.doctor_id.id),
                     '|', '|', '|',
                     '&',
                     ('start_time', '>', cur_start),
                     ('start_time', '<', cur_finish),
                     '&',
